...
@section('content')
    <form action="{{ route('dashboard.update-study-day', $studyDay->id) }}" method="POST" class="w-[80%] flex flex-col items-center gap-y-2 rounded-3xl p-4 bg-white outline-2 outline-lightBorder shadow-xl">
        ...
        <h1 class="text-[1.5rem] font-semibold">Edit a Schedule</h1>
        ...
        <div class="input-wrapper w-full flex flex-col rounded-2xl p-4">
            ...
            <textarea name="description" id="description" placeholder="Enter description ..." cols="10" rows="5" class="bg-lightPrimary p-1 w-full text-base placeholder:text-gray-400 focus:outline-lightAccent focus:outline-2 rounded-lg">{{ $studyDay->description }}</textarea>
        </div>
